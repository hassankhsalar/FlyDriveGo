...
const RoadSection = ({ roadData }) => {
  ...
  const filteredRoad = applyFilters(roadData);

  return (
    <section className="mb-12">
      <h2 className="text-2xl font-semibold mb-4 text-CharcoleDark">By Road</h2>
      <div className="flex flex-col md:flex-row gap-6">
        <FilterPanel filters={filters} setFilters={setFilters} section="road" />
        <div className="flex-1 md:mt-[-54px]">
          <SortDropdown sortKey={sortKey} setSortKey={setSortKey} />
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredRoad.map((option) => (
